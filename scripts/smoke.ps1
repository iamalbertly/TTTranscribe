...
do {
  ...
  try {
    $null = Invoke-RestMethod -Uri http://127.0.0.1:8000/health -Method GET -TimeoutSec 3 -SkipHttpErrorCheck
    $serverUp = $true
...
try {
  ...
  $resp = Invoke-RestMethod -Uri http://127.0.0.1:8000/transcribe -Method POST -Body $body -ContentType 'application/json' -TimeoutSec 10 -SkipHttpErrorCheck
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    $jr = Invoke-RestMethod -Uri ("http://127.0.0.1:8000/transcribe/$job") -Method GET -TimeoutSec 10 -SkipHttpErrorCheck
  ...
  $resp2 = Invoke-RestMethod -Uri http://127.0.0.1:8000/transcribe -Method POST -Body $body -ContentType 'application/json' -TimeoutSec 10 -SkipHttpErrorCheck
  $job2 = $resp2.job_id
  $deadline2 = (Get-Date).AddMinutes(3)
  while ((Get-Date) -lt $deadline2) {
    Start-Sleep -Seconds 2
    $jr2 = Invoke-RestMethod -Uri ("http://127.0.0.1:8000/transcribe/$job2") -Method GET -TimeoutSec 10 -SkipHttpErrorCheck
